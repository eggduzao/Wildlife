"""setuptools based setup script for Wildlife.
...
Wildlife source code, or fetched it from git, for the simplest
installation just type the command::

    python setup.py install

However, you would normally install the latest Wildlife release from
the PyPI archive with::

    pip install wildlife

For more in-depth instructions, see the installation section of the
Wildlife manual, linked to from:

http://wildlife.org/wiki/Documentation

Or, if all else fails, feel free to write to the sign up to the Wildlife
mailing list and ask for help.  See:

http://wildlife.org/wiki/Mailing_lists
"""
...
if sys.version_info[:2] < MIN_PY_VER:
    sys.stderr.write(
        ("ERROR: Wildlife requires Python %i.%i or later. " % MIN_PY_VER)
        + ("Python %d.%d detected.\n" % sys.version_info[:2])
    )
    sys.exit(1)


class test_wildlife(Command):
    ...
    description = "Automatically run the test suite for Wildlife."
...
setup(
    name="wildlife",
    version=__version__,
    author="The Wildlife Contributors",
    author_email="wildlife@wildlife.org",
    url="https://wildlife.org/",
    description="Freely available tools for computational molecular biology.",
    long_description=readme_rst,
    project_urls={
        "Documentation": "https://wildlife.org/wiki/Documentation",
        "Source": "https://github.com/wildlife/wildlife/",
        "Tracker": "https://github.com/wildlife/wildlife/issues",
    },
    classifiers=[
        ...
        # Technically the "Wildlife License Agreement" is not OSI approved,
