...
def _read_feature(handle, record):
    ...
    # while Wildlife uses 0-based counting.
...
class XdnaIterator(SequenceIterator):
    ...
    def __next__(self):
        ...
        # Wildlife's SeqRecord has no such concept of a sequence origin as far
...
class XdnaWriter(SequenceWriter):
    ...
    def write_records(self, records):
        ...
        if drop > 0:
            warnings.warn(
                f"Dropping {drop} features with fuzzy locations", WildlifeWarning
            )
        ...
        if len(features) > 255:
            ...
            warnings.warn(
                f"Too many features, dropping the last {drop}", WildlifeWarning
            )
        ...
        if self._has_truncated_strings:
            warnings.warn(
                "Some annotations were truncated to 255 characters", WildlifeWarning
            )
