...
def _insdc_feature_position_string(pos, offset=0):
    ...
    if isinstance(pos, SeqFeature.ExactPosition):
        ...
        raise NotImplementedError("Please report this as a bug in Wildlife.")
    else:
...
def _insdc_location_string_ignoring_strand_and_subfeatures(location, rec_length):
    ...
    if (
        ...
        if isinstance(location.start, SeqFeature.UnknownPosition) and isinstance(
            ...
            # warnings.warn("Feature with unknown location", WildlifeWarning)
            # return "?"
...
class _InsdcWriter(SequenceWriter):
    ...
    def _write_feature(self, feature, record_length):
        ...
        self.handle.write(line)
        # Now the qualifiers...
        # Note as of Wildlife 1.69, this is an ordered-dict, don't sort it:
...
class GenBankWriter(_InsdcWriter):
    ...
    def _write_single_line(self, tag, text):
        ...
        if len(text) > self.MAX_WIDTH - self.HEADER_WIDTH:
            if tag:
                warnings.warn(
                    f"Annotation {text!r} too long for {tag!r} line", WildlifeWarning
                )
            else:
                # Can't give such a precise warning
                warnings.warn(f"Annotation {text!r} too long", WildlifeWarning)
        self.handle.write(
    ...
    def _write_the_first_line(self, record):
        ...
        if mol_type and len(mol_type) > 7:
            ...
            if len(mol_type) > 7:
                warnings.warn(f"Molecule type {mol_type!r} too long", WildlifeWarning)
                mol_type = "DNA"
    ...
    def write_record(self, record):
        ...
        # and discussion https://github.com/wildlife/wildlife/pull/616
...
class EmblWriter(_InsdcWriter):
    ...
    def _write_single_line(self, tag, text):
        ...
        if len(text) > self.MAX_WIDTH:
            warnings.warn(f"Line {line!r} too long", WildlifeWarning)
        self.handle.write(line + "\n")
    ...
    def _write_the_first_lines(self, record):
        ...
        if mol_type not in (
            ...
            warnings.warn(f"Non-standard molecule type: {mol_type}", WildlifeWarning)
