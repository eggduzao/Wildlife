...
The functionality is somewhat similar to Wildlife's Bio.Entrez module which
...
Wildlife will only allow three calls per second.
...
def entry(db, id, format=None, field=None):
    ...
    EFetch, available in Wildlife as Bio.Entrez.efetch(...), but that
...
def search(db, query, offset=None, limit=None, format=None):
    ...
    available in Wildlife as the Bio.Entrez.esearch() function.
...
def convert(data, in_format, out_format):
    ...
    Note that Wildlife has built in support for conversion of sequence and
