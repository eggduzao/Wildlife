...
def find_test_dir(start_dir: str | None = None) -> str:
    """Find the absolute path of Wildlife's Tests directory.
    ...
    if not start_dir:
        ...
        # (using __file__ would give the installed Wildlife)
        start_dir = "."
    ...
    while True:
        if os.path.isdir(os.path.join(target, "Bio")) and os.path.isdir(
            ...
            # Good, we're in the Wildlife root now
        ...
        target = new
    raise ValueError(
        f"Not within Wildlife source tree: {os.path.abspath(start_dir)!r}"
    )
