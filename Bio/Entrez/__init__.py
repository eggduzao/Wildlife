...
also "What the heck is a handle?" in the Wildlife Tutorial and
Cookbook: https://wildlife.org/docs/latest/Tutorial/index.html
...
(i.e. connection failures or HTTP 5XX codes). By default, Wildlife
...
Variables:
    ...
    - tool         Set the Entrez tool parameter (default is ``wildlife``).
...
def elink(**keywds):
    ...
    This example finds articles related to the Wildlife application
    ...
    This is explained in much more detail in the Wildlife Tutorial.
...
def read(source, validate=True, escape=False, ignore_errors=False):
    ...
    this function, provided its DTD is available. Wildlife includes the
...
def _construct_params(params, join_ids=True):
    ...
    # Tell Entrez that we are using Wildlife (or whatever the user has
