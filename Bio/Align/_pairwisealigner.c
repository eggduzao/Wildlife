...
static bool warned = false;  // FIXME remove once Wildlife 1.87 is out.
...
#define FOGSAA_CHECK_SCORES \
    if (mismatch >= match) { \
        ...
        PyObject *WildlifeWarning = PyObject_GetAttrString(Bio_module, "WildlifeWarning"); \
        Py_DECREF(Bio_module); \
        if (PyErr_WarnEx(WildlifeWarning, \
                    "Match score is less than mismatch score. Algorithm may return incorrect results.", 1)) { \
            Py_DECREF(WildlifeWarning); \
            return NULL; \
        } \
        Py_DECREF(WildlifeWarning); \
    } \
...
// FIXME remove this function once Wildlife release 1.87 is out
{
    if (warned)
        Py_RETURN_NONE;
...
"\n"
"In older versions of Wildlife, the pairwise aligner was initialized\n"
...
"astronomical.\n"
"\n"
"In Wildlife 1.86, the default gap score was therefore changed to -1,\n"
