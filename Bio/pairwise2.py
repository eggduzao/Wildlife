...
- Two sequences: strings, Wildlife sequence objects or lists.
...
- The alignment function can also use known matrices already included in
  Wildlife (in ``Bio.Align.substitution_matrices``):
...
warnings.warn(
    ...
    "future release of Wildlife. As an alternative, please consider using "
    "Bio.Align.PairwiseAligner as a replacement, and contact the "
    "Wildlife developers if you still need the Bio.pairwise2 module.",
    WildlifeDeprecationWarning,
)
...
strings, lists or Wildlife sequence objects.\n
"""
            if match_doc:
                doc += f"\n{match_doc}\n"
            if penalty_doc:
...
def format_alignment(align1, align2, score, begin, end, full_sequences=False):
    ...
    Since Wildlife 1.71 identical matches are shown with a pipe
