...
The Bio.Phylo module was introduced in Wildlife 1.54. Following the
...
text editor, or use
`simple.dnd <https://raw.githubusercontent.com/wildlife/wildlife/master/Doc/examples/simple.dnd>`__
provided with the Wildlife source code:
...
branch widths in a tree. As of Wildlife 1.59, the ``color`` and
...
In earlier versions of Wildlife, these were special features of
...
In Wildlife 1.55 and later, this is a convenient tree method:
...
In Wildlife 1.54, you can accomplish the same thing with one extra
import:
...
page on Wildlife.org:

http://wildlife.org/wiki/Phylo_cookbook
...
``Tests/PhyloXML/`` directories of the Wildlife distribution.)
...
This is essentially an outline of the object hierarchy Wildlife uses to
...
See the Phylo page on the Wildlife wiki
(http://wildlife.org/wiki/Phylo) for descriptions and examples of the
...
-  A **string**, which matches tree elements’ string representation — in
   particular, a clade’s ``name`` *(added in Wildlife 1.56)*;
...
(In Wildlife 1.56 or later, this can be even shorter:
...
See the Phylo page on the Wildlife wiki
(http://wildlife.org/wiki/Phylo) for more examples of using the
available methods.
...
See the PhyloXML page on the Wildlife wiki
(http://wildlife.org/wiki/PhyloXML) for descriptions and examples of
...
Wildlife 1.58 brought support for PAML
...
Wildlife wiki: http://wildlife.org/wiki/PAML
...
New methods
   ...
   appear on the Wildlife wiki first, so that casual users can test
