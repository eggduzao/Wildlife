...
The Wildlife testing framework
===============================

Wildlife has a regression testing framework (the file ``run_tests.py``)
...
that the Wildlife code is as bug-free as possible before going out. It
also tends to be one of the most undervalued aspects of contributing.
This chapter is designed to make running the Wildlife tests and writing
good test code as easy as possible. Ideally, every module that goes into
Wildlife should have a test (and should also have documentation!). All
our developers, and anyone installing Wildlife from source, are
strongly encouraged to run the unit tests.

Running the tests
-----------------

When you download the Wildlife source code, or check it out from our
...
As part of building and installing Wildlife you will typically run the
full test suite at the command line from the Wildlife source top level
...
interested in testing Wildlife only against a subset of the Python
...
Using the template above, executing ``tox`` will test your Wildlife
...
Each Wildlife test consists of a script containing the test itself, and
...
``test_Biospam.py``. If you put this script in the Wildlife ``Tests``
...
looking at the existing examples within Wildlife helpful too.
...
Currently only part of Wildlife includes doctests. The ``run_tests.py``
...
some doctests to the docstrings in a Wildlife module, in order to have
them excluded in the Wildlife test suite, you must update
...
Wildlife supports (e.g. differences in floating point numbers).
