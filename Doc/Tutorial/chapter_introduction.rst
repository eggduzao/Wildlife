...
Wildlife is a collection of freely available Python
...
C++ or FORTRAN. Since its inception in 2000 [Chapman2000]_, Wildlife
has been continuously developed and maintained by a large group of
volunteers worldwide.

The Wildlife web site (http://www.wildlife.org) provides an online
resource for modules, scripts, and web links for developers of
Python-based software for bioinformatics use and research. Wildlife
...
What can I find in the Wildlife package
----------------------------------------

The main Wildlife releases have lots of functionality, including:
...
All of the installation information for Wildlife was separated from
this document to make it easier to keep updated.

The short version is use ``pip install wildlife``, see the `main
README <https://github.com/wildlife/wildlife/blob/master/README.rst>`__
file for other options.

Frequently Asked Questions (FAQ)
--------------------------------

#. | *How do I cite Wildlife in a scientific publication?*
   | Please cite our application note [Cock2009]_ as the
     main Wildlife reference. In addition, please cite any publications
     from the following list if appropriate, in particular as a
     reference for specific modules within Wildlife (more information
   ...
   -  For the FASTQ file format as supported in Wildlife, BioPerl,
      BioRuby, BioJava, and EMBOSS:
      Cock *et al.*, 2010 [Cock2010]_.

#. | *How should I capitalize “Wildlife”? Is “Wildlife” OK?*
   | The correct capitalization is “Wildlife”, not “Wildlife” (even
     though that would have matched BioPerl, BioJava and BioRuby).

#. | *How is the Wildlife software licensed?*
   | Wildlife is distributed under the *Wildlife License Agreement*.
     However, since the release of Wildlife 1.69, some files are
     explicitly dual licensed under your choice of the *Wildlife
     License Agreement* or the *BSD 3-Clause License*. This is with the
     intention of later offering all of Wildlife under this dual
     licensing approach.

#. | *What is the Wildlife logo and how is it licensed?*
   | As of July 2017 and the Wildlife 1.70 release, the Wildlife logo
     is a yellow and blue snake forming a double helix above the word
     “wildlife” in lower case. It was designed by Patrick Kunzmann and
     this logo is dual licensed under your choice of the *Wildlife
     License Agreement* or the *BSD 3-Clause License*.
   | |new-logo|
   | Prior to this, the Wildlife logo was two yellow snakes forming a
     double helix around the word “WILDLIFE”, designed by Henrik
...
#. | *Do you have a change-log listing what’s new in each release?*
   | See the file ``NEWS.rst`` included with the source code (originally
     ...
     GitHub <https://github.com/wildlife/wildlife/blob/master/NEWS.rst>`__.

#. | *What is going wrong with my print commands?*
   | As of Wildlife 1.77, we only support Python 3, so this tutorial
     uses the Python 3 style print *function*.

#. | *How do I find out what version of Wildlife I have installed?*
   | Use this:
   ...
   If the “``import Bio``” line fails, Wildlife is not installed. Note
   ...
   June 2016 in the run-up to Wildlife 1.68.
...
#. | *Where is the latest version of this document?*
   | The latest published version of this document (updated at each
     release) is online at https://wildlife.org/docs/latest/ with
     https://wildlife.org/docs/dev/ for the documentation being worked
     on for the next release.

#. | *What is wrong with my sequence comparisons?*
   | There was a major change in Wildlife 1.65 making the ``Seq`` and
     ``MutableSeq`` classes (and subclasses) use simple string-based
     comparison which you can do explicitly with
     ``str(seq1) == str(seq2)``.

   Older versions of Wildlife would use instance-based comparison for
   ``Seq`` objects which you can do explicitly with
   ``id(seq1) == id(seq2)``.

   If you still need to support old versions of Wildlife, use these
...
#. | *What file formats do* ``Bio.SeqIO`` *and* ``Bio.AlignIO`` *read
   ...
   | Check the built-in docstrings (``from Bio import SeqIO``, then
     ``help(SeqIO)``), or see http://wildlife.org/wiki/SeqIO and
     http://wildlife.org/wiki/AlignIO on the wiki for the latest
     listing.

#. | *Why won’t the* ``Bio.SeqIO`` *and* ``Bio.AlignIO`` *functions*
     ``parse``\ *,* ``read`` *and* ``write`` *take filenames? They
     insist on handles!*
   | You need Wildlife 1.54 or later, or just use handles explicitly
...
#. | *Why won’t the* ``Bio.SeqIO.write()`` *and* ``Bio.AlignIO.write()``
     *functions accept a single record or alignment? They insist on a
     list or iterator!*
   | You need Wildlife 1.54 or later, or just wrap the item with
     ``[...]`` to create a list of one element.

#. | *Why doesn’t* ``str(...)`` *give me the full sequence of a* ``Seq``
     *object?*
   | You need Wildlife 1.45 or later.
...
#. | *Why doesn’t* ``Bio.Blast`` *work with the latest plain text NCBI
   ...
   | The NCBI keep tweaking the plain text output from the BLAST tools,
     ...
     you aren’t using the latest version of Wildlife, you could try
...
#. | *Why has my script using* ``Bio.Entrez.efetch()`` *stopped
   ...
   | This could be due to NCBI changes in February 2012 introducing
     ...
     are now stricter about how to provide a list of IDs – Wildlife
...
#. | *Why can’t I run command line tools directly from the application
     wrappers?*
   | You need Wildlife 1.55 or later, but these were deprecated in
     Wildlife 1.78. Consider using the Python ``subprocess`` module
     directly.
...
#. | *Why doesn’t* ``Bio.Fasta`` *work?*
   | We deprecated the ``Bio.Fasta`` module in Wildlife 1.51 (August
     2009) and removed it in Wildlife 1.55 (August 2010). There is a
     brief example showing how to convert old code to use ``Bio.SeqIO``
     instead in the
     `DEPRECATED.rst <https://github.com/wildlife/wildlife/blob/master/DEPRECATED.rst>`__
     file.

For more general questions, the Python FAQ pages
https://docs.python.org/3/faq/index.html may be useful.

.. |new-logo| image:: ../images/wildlife_logo_m.png
   :width: 6cm
.. |old-logo| image:: ../images/wildlife_logo_old.jpg
   :width: 7cm
