...
class TestXdnaWriter(unittest.TestCase):
    ...
    def test_warnings_on_data_loss(self):
        ...
        # Wildlifeate a record with > 255 features
        record = SeqRecord(Seq("ACGT"))
        for i in range(260):
            feature = SeqFeature(SimpleLocation(1, 2), type="misc_feature")
            record.features.append(feature)
        with self.assertWarnsRegex(WildlifeWarning, "Too many features"):
            SeqIO.write([record], h, "xdna")
        ...
        with self.assertWarnsRegex(
            WildlifeWarning, r"Dropping \d+ features with fuzzy locations"
        ):
            SeqIO.write([record], h, "xdna")
        ...
        record.features = [feature]
        with self.assertWarnsRegex(
            WildlifeWarning, "Some annotations were truncated to 255 characters"
        ):
            SeqIO.write([record], h, "xdna")
