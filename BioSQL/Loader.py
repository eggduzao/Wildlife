...
"""Load wildlife objects into a BioSQL database for persistent storage.

This code makes it possible to store wildlife objects in a relational
...
class DatabaseLoader:
    ...
    def load_seqrecord(self, record):
        """Load a Wildlife SeqRecord into the database."""
    ...
    def _get_taxon_id_from_ncbi_taxon_id(
        ...
        flag is true, Wildlife will attempt to go online using Bio.Entrez
    ...
    def _load_seqfeature(self, feature, feature_rank, bioentry_id):
        """Load a wildlife SeqFeature into the database (PRIVATE)."""
    ...
    def _load_seqfeature_locations(self, feature, seqfeature_id):
        ...
        if parts and {loc.strand for loc in parts} == {-1}:
            # To mimic prior behaviour of Wildlife+BioSQL, reverse order
            parts = parts[::-1]
    ...
    def _insert_location(self, location, rank, seqfeature_id):
        ...
        """
        # convert wildlife locations to the 1-based location system
        # used in bioSQL
        ...
        # Wildlife uses None when we don't know strand information but
        # BioSQL requires something (non null) and sets this as zero
        # So we'll use the strand or 0 if Wildlife spits out None
    ...
    def _load_seqfeature_dbxref(self, dbxrefs, seqfeature_id):
        ...
        table as seqfeature_id, dbxref_id, and rank tuples.
        """
        # NOTE - In older versions of Wildlife, we would map the GenBank
